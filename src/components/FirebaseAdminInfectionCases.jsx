...
export function SummaryRow({ items }) {
  return (
    <section className="grid gap-2 sm:grid-cols-3 lg:grid-cols-5" aria-label="감염병 사례 요약">
      {items.map((item) => (
        <article key={item.status} className="flex min-h-14 items-center justify-between gap-3 rounded-[12px] border border-[#DDEAE7] bg-white px-3 py-2 shadow-[var(--shh-soft-shadow)]">
...
function StatusChip({ label, tone = "info" }) {
  ...
  return <span className={`rounded-[8px] border px-2.5 py-1 text-xs font-semibold ${className}`}>{label}</span>;
}
...
function Recommendation({ infectionCase }) {
  ...
  return (
    <p className="rounded-[8px] border border-[#F2D6B3] bg-[#FFF8ED] px-3 py-2 text-xs font-semibold text-[#8A4B12]">
...
function CaseStatusSelect({ caseStatus, disabled, onChange }) {
  return (
    <select
      ...
      disabled={disabled}
      className="min-h-10 rounded-[9px] border border-[#DDEAE7] bg-white px-3 py-2 text-sm font-semibold text-[#102047] outline-none focus:ring-4 focus:ring-[#20A982]/15 disabled:cursor-not-allowed disabled:opacity-50"
    >
    ...
    <article className="rounded-[12px] border border-[#DDEAE7] bg-white p-4 shadow-[var(--shh-soft-shadow)]">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <h2 className="mt-3 text-base font-semibold leading-6 text-[#102047]">
...
export function InfectionCaseFilters({
  ...
  return (
    <div className="grid gap-3 md:grid-cols-[minmax(150px,1fr)_120px_120px_minmax(180px,1.4fr)_auto] md:items-end">
      ...
      <label className="inline-flex min-h-10 items-center gap-2 rounded-[9px] border border-[#DDEAE7] bg-[#F3F8F6] px-3 py-2 text-sm font-semibold text-[#102047]">
        <input
          type="checkbox"
