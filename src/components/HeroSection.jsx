  ...
  return (
    <section id="home" className="mx-auto w-full max-w-6xl px-3 pb-2 pt-3 sm:px-4 lg:max-w-[1280px]">
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1.52fr)_minmax(380px,0.68fr)] lg:items-stretch">
        <div className="relative overflow-hidden rounded-[14px] border border-[#D5E8F8] bg-[linear-gradient(135deg,#F4FAFF_0%,#EEF7FF_56%,#FFF8FB_100%)] px-4 py-5 text-[#102047] shadow-[0_8px_24px_rgba(16,32,71,0.06)] sm:px-6 sm:py-6 lg:min-h-[244px] lg:px-8 lg:py-7">
          ...
          <div className="relative z-10 max-w-[700px]">
            ...
            <h1
              className="mt-4 max-w-[620px] text-[1.75rem] font-bold leading-[1.18] text-[#102047] sm:text-[2rem] lg:text-[2.15rem]"
              style={{
                wordBreak: "keep-all",
                overflowWrap: "normal",
                letterSpacing: "0"
            ...
            {notices.length > 0 && (
              <div className="mt-4 grid w-full max-w-full grid-cols-[20px_minmax(0,1fr)] items-start gap-2 rounded-[12px] border border-[#C8D8FF] bg-white/85 px-3 py-1.5 text-[11px] font-normal leading-5 text-[#3154A3] shadow-[0_1px_2px_rgba(16,32,71,0.04)] sm:w-fit sm:max-w-[640px] sm:items-center sm:rounded-full sm:text-xs lg:max-w-[660px]">
                ...
                <span className="min-w-0" style={{ wordBreak: "normal", overflowWrap: "anywhere" }}>
                  {notices.join(" ")}
                </span>
              </div>
            )}
          </div>
          <div className="pointer-events-none absolute bottom-0 right-5 hidden h-32 w-32 overflow-hidden rounded-full border border-white/75 bg-white/70 shadow-[0_10px_24px_rgba(16,32,71,0.12)] md:block lg:h-40 lg:w-40">
            <img
              src={MASCOT_SRC}
              alt=""
              aria-hidden="true"
        ...
        {action && (
          <div className="min-w-0 self-stretch lg:min-h-[244px]">
            {action}
          </div>
        )}
      </div>
