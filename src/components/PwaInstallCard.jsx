...
export default function PwaInstallCard() {
  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-3 pb-2 sm:px-4 lg:max-w-[1280px]">
      <details className="group overflow-hidden rounded-[14px] border border-[#DDEAE7] bg-white/82 transition hover:border-[#BFEBDC]">
        <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-2 px-3 py-1.5 marker:content-none sm:gap-3 sm:px-4 sm:py-2">
          ...
          <span className="shrink-0 text-xs font-bold text-[#08754B] group-open:hidden">자세히 보기 →</span>
          <span className="hidden shrink-0 text-xs font-bold text-[#08754B] group-open:inline">접기</span>
        </summary>
        <div className="border-t border-[#DDEAE7] px-3 pb-3 sm:px-4">
          ...
          <p className="mt-2 rounded-[10px] bg-[#FFF7F7] px-3 py-2 text-xs font-semibold leading-5 text-[#B42318]" style={{ wordBreak: "keep-all" }}>
          ...
          {canUseInstallPrompt && installPrompt && !isInstalled && (
            <button
              ...
              className="mt-3 min-h-10 w-full rounded-[10px] bg-[#20A982] px-4 py-2 text-xs font-semibold text-white transition hover:-translate-y-[1px] hover:bg-[#178C6C] sm:w-auto"
            >
              앱 설치
            </button>
          )}
