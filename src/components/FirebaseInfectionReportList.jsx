...
export default function FirebaseInfectionReportList({ reports, state }) {
  if (state.status === "loading") {
    return (
      <p className="rounded-[24px] bg-white/80 p-5 text-sm font-semibold text-[#627083]">
        보고 목록을 불러오는 중입니다.
      </p>
    );
  }
  ...
  if (state.status === "permission-denied" || state.status === "error") {
    return <p className="rounded-[24px] bg-[#FFF7F7] p-5 text-sm font-semibold text-[#B42318]">{state.message}</p>;
  }
